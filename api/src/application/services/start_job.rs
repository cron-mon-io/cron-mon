...
pub struct StartJobService<
    ...
    ApiKeyRepo: Repository<ApiKeyData> + GetByKey,
> {
    monitor_repo: MonitorRepo,
    api_key_repo: ApiKeyRepo,
}

impl<MonitorRepo: Repository<Monitor>, ApiKeyRepo: Repository<ApiKeyData> + GetByKey>
    StartJobService<MonitorRepo, ApiKeyRepo>
{
    ...
    pub async fn start_job_for_monitor(
        ...
        let key = self.api_key_repo.get_by_key(api_key).await?;

        let mut monitor_opt = self.monitor_repo.get(monitor_id, Some(key.tenant)).await?;
