...
impl<MonitorRepo: GetWithLateJobs + Repository<Monitor>, Notifier: NotifyLateJob>
    ProcessLateJobsService<MonitorRepo, Notifier>
{
    pub fn new(monitor_repo: MonitorRepo, notifier: Notifier) -> Self {
        Self {
            monitor_repo,
            notifier,
        }
    ...
    pub async fn process_late_jobs(&mut self) -> Result<(), Error> {
        ...
        for mon in monitors_with_late_jobs.as_mut_slice() {
            ...
            self.monitor_repo.save(mon).await?;
        }
...
mod tests {
    ...
    async fn test_process_late_jobs_service() {
        ...
        let mut service = ProcessLateJobsService::new(mock_monitor_repo, mock_notifier);
