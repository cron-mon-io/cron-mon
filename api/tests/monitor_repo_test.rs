...
async fn test_all_with_tenant() {
    ...
    let montiors = repo.all(Some("foo".to_owned())).await.unwrap();
...
async fn test_save() {
    ...
    assert_eq!(repo.all(Some("foo".to_owned())).await.unwrap().len(), 4);

    let read_new_monitor = repo
        .get(new_monitor.monitor_id, Some("foo".to_owned()))
        .await
        .unwrap()
        .unwrap();
...
async fn test_delete() {
    ...
    assert!(repo
        .get(monitor.monitor_id, Some("foo".to_owned()))
        .await
        .unwrap()
        .is_none());
    assert_eq!(repo.all(Some("foo".to_owned())).await.unwrap().len(), 2);
}
...
async fn test_loading_invalid_job() {
    ...
    let monitor_result = repo
        .get(
            gen_uuid("027820c0-ab21-47cd-bff0-bc298b3e6646"),
            Some("foo".to_owned()),
        )
        .await;
